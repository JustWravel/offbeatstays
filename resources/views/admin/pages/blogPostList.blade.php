							...
							<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
								...
								<div class="d-flex align-items-center flex-wrap mr-1">
									...
									<div class="d-flex align-items-baseline flex-wrap mr-5">
										...
										<!--begin::Page Title-->
										<h5 class="text-dark font-weight-bold my-1 mr-5">All Blog Posts</h5>
								...
								<div class="d-flex align-items-center">
									...
									<!--begin::Actions-->
									<a href="{{ route('admin.blogpost.create') }}" class="btn btn-light-primary font-weight-bolder btn-sm">Add New</a>
								...
								<div class="card card-custom">
									<div class="card-header flex-wrap py-5">
										<div class="card-title">
											<h3 class="card-label">View All Blog Posts
											</h3>
										</div>
									</div>
									<div class="card-body">
										...
										<table class="table table-separate table-head-custom table-checkable" id="kt_datatable">
											...
											<tbody>
												...
												<tr>
													<td>{{ $blogpost->id }}</td>
													<td><a href="{{ $blogpost->slug }}">{{ $blogpost->name }}</a></td>
													<td><img src="{{ $blogpost->image }}" style="width: 40px" alt=""></td>
													{{-- <td>{{ $blogpost->description }}</td> --}}
													
													<td nowrap="nowrap">
														<a href="{{ route('admin.blogpost.edit', ['blogpost' => $blogpost->id]) }}" class="btn btn-sm btn-primary btn-icon" title="Edit details">
															<i class="la la-edit"></i>
														</a>
														<form method="POST" action="{{ route('admin.blogpost.show', ['blogpost' => $blogpost->id]) }}" onsubmit="return confirm('Are you sure ?? Delete this blogpost!')" style="display: inline;">
														    @csrf
														    @method('DELETE')
														    <button type="submit" class="btn btn-sm btn-danger btn-icon" title="Delete"><i class="la la-trash"></i></button>
														</form>
														{{-- <a href="{{ route('admin.blogpost.show', ['blogpost' => $blogpost->id]) }}" class="btn btn-sm btn-danger btn-icon" title="Delete">
